...
const UsersLeaderboardPage: NextPage = () => {
  ...
  const [topThree, setTopThree] = useState<PersonData[] | null>(null);
        ...
        <div className='w-[calc(100%-20px)] md:w-[80%] m-auto grid grid-flow-row gap-2'>
          <div className='w-full grid grid-cols-[repeat(3,minmax(0,1fr))] gap-[6px] mt-2 lg:mt-6'>
            {topThree.length !== 0 && (
              <>
                {topThree.length >= 2 && (
                  <LeaderboardTop3 name={topThree[1][1].Name} email={topThree[1][0]} moneyRaised={topThree[1][1].MoneyRaised} rank={2} animationDelay={200} />
                )}
                {topThree.length >= 1 && (
                  <LeaderboardTop3 name={topThree[0][1].Name} email={topThree[0][0]} moneyRaised={topThree[0][1].MoneyRaised} rank={1} animationDelay={300} />
                )}
                {topThree.length >= 3 && (
                  <LeaderboardTop3 name={topThree[2][1].Name} email={topThree[2][0]} moneyRaised={topThree[2][1].MoneyRaised} rank={3} animationDelay={100} />
                )}
              </>
            )}
          </div>
